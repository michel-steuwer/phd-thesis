...
\bigskip\noindent
In the following, we describe how the parallelization schema phases in \autoref{fig:lmosem:em_distribution} are implemented using OpenCL.
...
\paragraph{Step 1}
The implementation of step 1 performs the operations shown in \autoref{lst:lmosem:seq_code}: first computing $A_i$, then the local error for $A_i$ and finally adding the local error to $c_l$.
