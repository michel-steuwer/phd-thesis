...
We create the two input \code{Vector}s from C style pointers in lines~\autoref{lst:skelcl:dotproduct:vecA} and \autoref{lst:skelcl:dotproduct:vecB}.
...
The user is free to write expression~\ref{lst:skelclc:after} explicitly, but it is arguably more convenient to write expression~\ref{lst:skelclc:before} and let our custom compiler perform the transformation automatically.

There exist a function like \code{zip} for every skeleton available in \SkelCL which is transformed by \code{skelclc} to a call of the constructor of the corresponding class.
...
In \autoref{lst:skelcl:after} the type \code{Container<float>} is used as input types and result type for the template arguments of the \zip skeleton.
The skeletons \map, \zip, and \stencil can operate on \code{Vector} and \code{Matrix}.
...
If the result type of \code{zip} would not match the input type of \code{reduce} an compile time error would occur.
