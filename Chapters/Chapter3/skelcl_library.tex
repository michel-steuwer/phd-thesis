...
The skeletons are created using \code{zipVector} (line~\autoref{lst:skelcl:dotproduct:zip}) and \code{reduce} (line~\autoref{lst:skelcl:dotproduct:reduce}) functions which expect the customizing functions of the skeletons as arguments.
By using the \code{zipVector} function we specify that the \zip skeleton operates on two input vectors, rather than on two input matrices.
...
We create the two input \code{Vector}s from C style pointers (lines~\autoref{lst:skelcl:dotproduct:vecA} and \autoref{lst:skelcl:dotproduct:vecB}).
...
the interface of the \code{Vector} class looks familiar to \Cpp programmers and the usage of modern \Cpp features like lambda expressions and type deduction (\code{auto}) simplify the programming as functions can be defined directly inline and type information can be omitted.
...
In this example, the \code{zipVector} function has been transformed into a call of the constructor of the \code{Zip} class and the lambda expression has been transformed into a string.
...
We observed that loading kernels from disk is at least five times faster than creating them from the source.
...
\paragraph{The \code{MapOverlap} implementation}
...
The \code{mapOverlap} factory function is customized with a lambda expression which is passed a \code{Neighborhood} object used to access the neighboring elements with relative indices.
...
\paragraph{The \code{Stencil} implementation}
...
The \code{Stencil} implementation supports the iterative execution, which is especially challenging to implement on multi device systems as we will see later.
...
The \code{Stencil} implementation uses a different strategy than the \code{MapOverlap} implementation in order to enable the usage of different padding modes and stencil shapes when using several \stencil skeletons in a sequence, which we discuss in the next paragraph.
...
This cannot be implemented efficiently using \code{MapOverlap}'s implementation strategy, as the input matrix is extended on the \CPU conforming to the stencil shape.
...
The single steps are combined into a single object \code{StencilSequence} which can be executed like a \stencil skeleton.
...
The \code{Stencil} implementation explicitly supports iterations and, therefore, only exchanges elements from the border region and does not perform a full down- and upload of the matrix, as the \code{MapOverlap} implementation does.
...
By using the fast but small local memory, this number of global memory accesses can be reduced and, thus, performance can be improved.
...
When the data of the container has to be transferred to or from the devices, the data distribution object is invoked to perform the operation.
