...
In the \SkelCL library implementation the \reduce skeleton returns a vector containing a single element, \code{result} in this particular case.
...
We could have also used the equivalent \code{front} member function for accessing the first element of the result vector.
...
To validate this explanation and quantify the effect of launching additional kernels where a single kernel would be sufficient we measured the performance of \SkelCL's \reduce skeleton on its own.
