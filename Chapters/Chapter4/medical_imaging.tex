...
The two computational steps are implemented using the \map and \zip skeleton from \SkelCL.

The first step -- the computation of the error image $c_l$ -- is implemented using the \map skeleton.
For each event \code{e} of the currently processed subset the $A_i$ is computed (line~\ref{lst:em:skelcl:computeAi}).
As $A_i$ is very sparse it is stored as a special data structure, called \code{Path}, to reduce the memory footprint.
...
The error image is also provided as additional argument, but when calling the \map skeleton \code{cl} is wrapped using the \code{out} helper function.
