...
For arrays the number of dimensions and the size of each dimension is part of their types, as we will see.
...
we write function application using space and functions are often curried;
binary operators (\eg, $\oplus$) are written using infix notation and can be written using prefix notation when being sectioned using parenthesis: $(\oplus)$.
...
\begin{definition}
  ...
  \begin{align*}
    ...
    \id_\oplus &: \alpha,\\
    \vec{x} &: [\alpha]_n,\\
    \reduce\ \oplus\ \id_\oplus\ \vec{x} &: [\alpha]_1
  \end{align*}
\end{definition}
...
\begin{definition}
  ...
  \end{align*}
  The types of $\vec{x}$ and \join are as follows:
  \begin{align*}
...
The index function $F$ describes the effect the function $f$ has on the size of its input array.
This index function is used to compute the effect the \iterateN function has when applying $f$ $n$ times on its input array.
Please note, that the size of the input array of $f$, \ie, $k$, possibly changes every time $f$ is applied by \iterateN.
...
In fact, these patterns can be viewed as \emph{instantiations} of the \emph{interface} the high-level \map pattern define, as the low-level \OpenCL pattern provide different possibilities to implement the \map pattern.

\todo{describe map-warp and map-lane}
...
\begin{definition}
  ...
  \end{align*}
  where $\{x_1,\ldots,x_n\}$ denotes a vector of width $n$.\\
  The types of $\vec{x}$, and \asVector are as follows:
  \begin{align*}
