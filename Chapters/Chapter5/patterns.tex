...
The \pat{iterate} pattern iteratively applies a function multiple times.
Finally, the \pat{reorder} pattern lets our system know it is safe to reorder the elements of an array arbitrarily, which enables optimizations -- as we will see later in \autoref{chapter:codeGeneration-evaluation}.
...
For a function $f$ of type $(\alpha \rightarrow \beta)$ and an array $xs$ of type $[\alpha]_n$, the \pat{map} pattern computes an array of type $[\beta]_n$.
Formally we can write the type of \pat{map} as a typing judgment:
...
The formal type of our \pat{reduce} pattern is:
...
\paragraph{Zip}
The \pat{zip} pattern and the following \pat{split}/\pat{join} patterns transform the shape of the data which we store in the type system, \ie, number of dimensions and size of each dimension.
The \pat{zip} pattern fuses two arrays into an array of pairs.
The type of the \pat{zip} pattern is:
...
The formal type of the \pat{split} pattern is:
\begin{align}
...
For instance, the \pat{map-workgroup} pattern assigns work to an \OpenCL \emph{workgroup} with every workgroup applying the given function on an element of the input array.
Similarly, the \pat{map-local} pattern assigns work to a local work item inside a workgroup.
As workgroups are optional in OpenCL the \pat{map-global} pattern assigns work to a work item not organized in a workgroup.
...
Inside of the loop, a pointer is generated to partition the input array, so that every workgroup calls \pat{map-workgroup}'s customizing function on a different chunk of data.
