...
\end{rerule}
%
The \pat{id} function can acts as a copy operation; this is, \eg, useful for expressing copies to local memory when composed with the \toLocal \OpenCL pattern: $\toLocal\ id$.
...
They express the fact that consecutive \pat{split}-\pat{join} pairs and \pat{asVector}-\pat{asScalar} pairs are equivalent to the identity.
%
\begin{rerule}{lcl}
  \pat{join} \circ \pat{split}\ n        & \rightarrow & \pat{id}\\
  %\pat{split}\ n \circ \pat{join} \circ\ f\ \circ \pat{split}\ n        & \rightarrow & \pat{id}\\
  \pat{asScalar} \circ \pat{asVector}\ n & \rightarrow & \pat{id}
  \label{eq:algo:simpl}
\end{rerule}
...
\begin{subfigure}[b]{1\linewidth}
  \begin{mdframed}
    \vspace{-\bigskipamount}
    \begin{rerule*}{lcl}
      \pat{split}\ n \circ \pat{join}
            & \rightarrow & \pat{id}\\
      \pat{asVector}\ n \circ \pat{asScalar}
            & \rightarrow & \pat{id}\\
    \end{rerule*}
  \end{mdframed}
  \vspace{-1em}
...
For instance, it is only legal to nest a \pat{map-local} inside a \pat{map-workgroup} and it is not valid to nest a \pat{map-global} or another \pat{map-workgroup} inside a \pat{map-workgroup}.
...
This holds since a \pat{map-local} always exists within a \pat{map-workgroup} for which the local memory is defined in \OpenCL.
...
\end{rerule}
%
Vectorization is achieved by using the \pat{asVector} and corresponding \pat{asScalar} patterns, which changes the element type of an array and adjust the length accordingly.
...
\begin{subfigure}[b]{1\linewidth}
  \begin{mdframed}
    ...
    \begin{rerule*}{lclcl}
      \pat{map}\ f
        & \rightarrow &
          \pat{map-workgroup}\ f & | & \pat{map-local}\ f\\
        & | &
          \pat{map-global}\ f    & | & \pat{map-seq}\ f\\
    \end{rerule*}
  \end{mdframed}
  \vspace{-1em}
  \caption{Map}
...
\begin{subfigure}[b]{1\linewidth}
  \begin{mdframed}
    ...
    \begin{rerule*}{lcl}
      \pat{map-local}\ f
        & \rightarrow &
          \pat{toGlobal\ (map-local}\ f)\\  
      \pat{map-local}\ f
        & \rightarrow & \pat{toLocal\ (map-local}\ f)\\  
    \end{rerule*}
  \end{mdframed}
  \vspace{-1em}
