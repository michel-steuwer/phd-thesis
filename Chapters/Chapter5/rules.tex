...
For instance the vectorization rule, as we will see, is the only \todoU{place where we express}{sg: rule expressing?} the vectorization concept.
This is different from most previous \todoU{approaches}{sg: zu was?} that would produce a special vectorized version of different algorithmic patterns such as map or reduce.
...
\todo{lowering = \OpenCL-specific: Uniform!}
...
The following \autoref{eq:algo:reorder} shows that \todoU{if the data can be reordered arbitrarily}{sg: Bedingung unklar. Was bedeutet, wie prueft man sie??} then, it does not matter if we apply a function $f$ to each element before or after the reordering.
%
\begin{rerule}{lcl}
  \map\ f \circ \reorder
...
\end{rerule}
%
This allows us to nest map patterns in each other and, thus, maps the computation to the \todoU{thread hierarchy of the \OpenCL programming model, such as $\mapWorkgroup\ (\mapLocal\ f)$}{sg: unklar, man sieht es nicht aus (5.4)} as seen in our motivation example (\autoref{fig:codeex}).
...
From the algorithmic point of view, we first define a partial reduction \todoU{pattern \partRed}{sg: formal einfuehren?}.
...
\todoU{This definition}{sg: ist das definition oder eine rule, die beweisbar ist?} ensures that we end up with one unique element.
...
%
\todo{sg: kann man sie formal beweisen? Ein paar Beweise waeren schoen.}
\begin{rerule}{lcl}
  \partRed\ f\ z
    & \rightarrow &
      \reduce\ f\ z\\
...
\end{rerule}
%
\todo{sg: intuitiv ist es klar, die Frage ist, ob das alles semi-formal oder doch formal ist?}
The first possible derivation for partial reduction leads to the full reduction, \ie, \todoU{$m=1$}{sg: man sieht kein $m$ in der Regel.}.
...
Each single \todoU{rule states a single statement of the relationship of multiple algorithmic patterns}{sg: klingt komisch}.
The rules allow us to formalize different algorithmic \todoU{implementation strategies}{sg: welche stretegien gibt es, z.B.?}.
