...
The rules guaranty that all derived expressions are semantically equivalent.
...
\end{align}
%
The formal derivations for all expression are shown in \autoref{chapter:AppendixA} in \autoref{section:derivations}.
...
First the input array is split into chunks of size 128 and each work-group processes such a chunk of data.
128 corresponds to the work-group size we assumed for our implementations in \autoref{section:reduce:case-study}.
Inside of a work-group each work-item first copies a single data item into the local memory using the \textit{id} function to perform a copy nested inside the $\toLocal$ pattern.
Afterwards, the entire work-group performs an iterative reduction where in 7 steps (this equals $log_2(128)$) the data is further divided into pairs of two elements which are reduced sequentially by the work-items.
Finally, the computed result is copied back to the global memory.
...
Therefore, on these three architectures it is always beneficial to choose the second over the first implementation, which is exactly what our \OpenCL code generator does as we will see in the next section.
...
\end{figure}
%
\autoref{eq:reduce12} shows our second expression implementing parallel reduction, which closely resembles the third implementation of parallel reduction shown in \autoref{lst:reduce2}.
...
This reflect the \OpenCL implementation, where the processing of the last 64 elements was performed by a single warp.
...
The stride of the \reorderStride patterns have been computed based on the size of the array in each iteration step. 
...
As we assume a fixed work-group size of 128 work-items we statically know for \autoref{eq:reduce14} that only a single iteration step is required.
If a larger work-group size would be chosen, then the expression shown in \autoref{eq:reduce15} would reflect this by including the additional iteration steps required.
...
The first \reorderStride expression ensures that memory accesses are coalesced when the global memory is accessed.


\paragraph{Conclusions}
In this subsection we resembled the \OpenCL implementations presented in \autoref{section:reduce:case-study} by expressing them as compositions of our patterns.
Furthermore, the expressions presented are all derivable from a simple high-level expression describing the parallel summation.
...
By expressing highly specialized and optimized implementations we show how flexible and versatile our patterns and rules are.
We will see in the next section how these expressions are turned into \OpenCL code.
...
\subsubsection{Systematically Fusion of Patterns}
Before we look at how \OpenCL code is generated in the next section, we want to discuss one additional optimization: fusion of patterns.
...
Two algorithmic skeletons, \reduce and \map, where composed to express the application as shown in \autoref{eq:asum:patterns}.
\begin{align}
...
\SkelCL treats each algorithmic skeleton separately, thus, forcing the \map skeleton to write a temporary array result back to global memory, which greatly limits performance.
...
This sequence of transformations results in an expression which allows for a more optimal implementation since no temporary storage is required for the intermediate result.
...
Therefore, this lambda is used in a sequential implementation of reduction and can not be used as the customizing function for an entire parallel reduction.
Nevertheless, implements the final expression an parallel reduction, as the \map pattern is used with the \splitN and \join patterns to split the array divide-and-conquer style and the \map pattern is followed by a \reduce pattern customized with addition which can be implemented in parallel, as we know.
...
\subsection{Summary}
In this section we have introduced a set of rewrite rules which allows to safely and systematically rewrite expressions expressed with the patterns introduced earlier in \autoref{section:patterns}.
...
These low-level expressions matches hardware concepts expressible with \OpenCL such as mapping computation and data to the thread and memory hierarchy, exploit memory coalescing, and vectorization.
