...
In contrast, the index for writing only uses \code{l\_id} because the result is stored in local memory and, therefore, each work-group operates on a separate copy of \code{sdata1}.
This section of the code is finished by the \code{barrier} in line~\ref{lst:reduce11:lc1:barrier}, which is emitted for the \mapLocal pattern to ensure proper synchronization between the work-item in the work-group.
...
No for loop is emitted, as it is clear from that the size of the array is maximal 128 (from the definition of \code{size} in line~\ref{lst:reduce11:iter:size}) or smaller, as the variable is halved after each iteration (line~\ref{lst:reduce11:iter:size:update}).
...
In line~\ref{lst:reduce11:lc2:reduceStore} the result is stored to memory using the \code{out} pointer prepared by the \iterateN pattern.
Finally, for the \mapLocal pattern an \code{barrier} for synchronization is emitted.
...
We saw examples of generated \OpenCL code in the previous section.
...
We intend to apply well understood algorithms which are used for register allocation in compilers, like graph coloring~\cite{}.
