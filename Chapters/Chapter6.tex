...
lowercase letters (\eg, \code{x}) denote scalar variables, letters with an arrow (\eg, $\vec{x}$) denote arrays, and capitalized letters (\eg, $A$) denote 2D arrays.
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[b]{0.475\linewidth}
    \includegraphics[width=\linewidth]{PLDI/first\string_results\string_nv.pdf}
    \caption{Nvidia GPU}
    \label{fig:ref-compare-nv}
  \end{subfigure}
  \begin{subfigure}[b]{0.475\linewidth}
    \includegraphics[width=\linewidth]{PLDI/first\string_results\string_amd.pdf} 
    \caption{AMD GPU}
...
We also evaluated Thrust~\cite{thrust}, a library developed by Nvidia.% based on CUDA.

%The results for the Nvidia system in figure~\ref{fig:ref-compare-nv} show, that the BLAS implementations are the fastest followed closely by the version found automatically by our search, which outperforms all other hand written versions.
The results for the Nvidia system in figure~\ref{fig:ref-compare-nv} show that the version found automatically by our search outperforms all hand written versions and is very close to the performance of the BLAS implementations.
...
gemv A xs ys a b =
  zs = map (scal a $\circ$ dot xs) A
  map add (zip zs (scal b ys))
\end{lstlisting}
