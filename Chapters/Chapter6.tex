...
\autoref{lst:linear-algebra-expr} shows how we express vector scaling, sum of absolute values, dot product of two vectors and matrix vector multiplication using our high-level patterns.
...
Line~\ref{lst:linear-algebra-expr:gemv} shows matrix vector multiplication as defined in BLAS: $\vec{y} = \alpha A \vec{x} + \beta \vec{y}$.
To multiply the matrix $A$ with the vector $\vec{x}$, the \map pattern maps the computation of the dot-product with the input vector $\vec{x}$ to each row of the matrix $A$.
Afterwards the resulting vector is added to the scaled vector $\vec{y}$ using the \zip and \map patterns.
...
We use the BLAS OpenCL implementations written by AMD as our baseline for this evaluation since it is implemented in \OpenCL and functionally portable across all different platforms.
...
The computational functions $compD1$, $compD2$, $compCallOption$, and $compPutOption$ are not shown here since they only contain purely sequential code implementing the BlackScholes model.
This $BSComputation$ function is applied to all stock prices, stored in a vector $stockPrices$, using the \map pattern in line~\ref{lst:blackScholes:map}.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=.65\linewidth]{Plots/BlackScholes/bs_speedup.pdf}
  \caption{Performance of our approach relative to a portable \OpenCL implementation of the BlackScholes benchmark from Nvidia.}
...
The function \code{updateF} (line~\ref{lst:md:updateF}) updates the force $f$ influencing particle $p$ by computing and adding the force between a single particle and one of its neighbors.
...
For computing the force for all particles $ps$, we use the \zip pattern~(line~\ref{lst:md:zip}) to build a vector of pairs, where each pair combines a single particle with the indices of all of its neighboring particles ($p$ and $ns$ in line~\ref{lst:md:md}).
...
This application example shows that our patterns are flexible enough to implement more complicated applications.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=.65\linewidth]{Plots/MD/md_speedup.pdf}
  \caption{Performance of our approach relative to a portable \OpenCL implementation of the MD benchmark from the SHOC benchmark suite.}
