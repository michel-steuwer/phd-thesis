...
\CUDA and \OpenCL are the most popular approaches for programming \GPU systems.
...
qualifiers for pointers reflecting the four address spaces (as the \code{global} qualifier in lines~\ref{lst:openclExample:1} and~\ref{lst:openclExample:2} of \autoref{lst:openclExample}) and vector data types, like \code{float4}.
...
In \autoref{lst:openclExample} each work-item computes one item of the result matrix \code{c} of matrix multiplication by multiplying and summing up a row and column of the input matrices \code{a} and \code{b}.
...
Dijkstra identified the low-level \code{go to} statement as a major reason for programmers writing ``spaghetti code''.
In this famous letter~\cite{} he argued that instead programs should have a more rigid structure organizing code in procedures and using higher level control structures like \code{if} and \code{while}.
...
The new structures proposed to replace \code{go to} where suggested based on observations of common use cases -- or \emph{patterns} of use -- of the \code{go to} statement in sequential programming.
...
A \code{if A then B else C} statement has a clear high-level semantic which is easy to understand and reason about, for example is it clear to the programmer -- and the compiler -- that \code{B} and \code{C} can not both be executed.
This helps the programmer to understand the source code and the compiler to produce an optimized executable.
The equivalent code containing multiple \code{go to} statements obtains the same high-level semantic by combining the low-level operations with their low-level semantics.
...
Researchers have more recently suggested similar arguments to Dijkstra's for moving away from traditional parallel programming towards \emph{structured parallel programing}.
...
\subsection{State of the Art of Structured Parallel Programming}
  ...
  % skeleton libraries (MPI & multicore)
