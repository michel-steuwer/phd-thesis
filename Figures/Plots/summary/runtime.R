...
data <- read.table("runtime.csv", header=TRUE, sep=",")
...
for (b in levels(data$Benchmark)) {
  data[data$Benchmark == b,]$Sums <-
    sum(data[data$Program == "OpenCL" & data$Benchmark == b,]$Runtime)
}
...
labs <- labs(x = "", y = "Relative Runtime")

# create plot
plot <- ggplot(na.omit(data), aes(x=Program, y=Runtime/Sums, fill=Program)) +
...
# save plot to file
ggsave(file = "summary_runtime.pdf", width = 10, height = 4)
