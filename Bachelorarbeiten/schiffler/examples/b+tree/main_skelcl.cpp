...
#include "./kernel/kernel_gpu_skelcl_wrapper.h"
...
long transform_to_cuda(node * root, bool verbose){
  ...
  while( queue != NULL ) {
    ...
    if(verbose){
      printf("< location: %d,\tindices: ",k->location);
      for(int j=0; j<DEFAULT_ORDER+1; j++){
	printf("%2d ",k->indices[j]);
      }
      printf(",\tkeys: ");
      for(int j=0; j<DEFAULT_ORDER+1; j++){
	if(k->keys[j]==INT_MAX)
	  printf("MAX ");
	else if(k->keys[j]==INT_MIN)
	  printf("MIN ");
	else
	  printf("%3d ",k->keys[j]);
      }
      printf(",\tis_leaf: %d",k->is_leaf);
      printf(",\tnum_keys: %d >\n",k->num_keys);
    }	
	...
	char** argv ) 
{
  ...
  const char *output="output_skelcl.txt";
  FILE * pFile;
  ...
  if (commandFile==NULL) {
    ...
    interactive=true;
    // fputs ("Command File error",stderr); exit (1);
  }
  ...
  if(!interactive){
    ...
    if (result != lSize) {
      fputs ("Command file reading error",stderr); exit (3);
    }
  ...
  while (sscanf(commandPointer, "%c", &instruction) != EOF) {
	...
	if(interactive)
	  ...
	  //   printf("Can only start the SkelCL version\n");
	  //   //exit(0);
	  // }
	}
	...
	//SkelCL Kernel		
	kernel_gpu_skelcl_wrapper(      records,
					records_elem,
					knodes,
					knodes_elem,
