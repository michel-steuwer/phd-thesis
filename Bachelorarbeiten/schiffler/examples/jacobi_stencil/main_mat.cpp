...
#define FILE_OUT "output_mat.bin"
...
template <typename Iterator>
void initPositions(Iterator iter, int nx, int ny, int nz){
  for(int y=0; y<ny*nz; y++){
    for(int x=0; x<nx; x++){
      (*iter).x=x;
      (*iter).y=y;
      iter++;
...
int main(int argc, char** argv){
  ...
  pos = Matrix<pos_t>({ny*nz, nx});
  initPositions(pos.begin(), nx, ny, nz);
  ...
  Map<float(pos_t)> f(std::ifstream("kernel_mat.cl"));
