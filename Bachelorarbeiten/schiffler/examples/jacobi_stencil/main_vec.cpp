...
#define FILE_OUT "output_vec.bin"
...
int main(int argc, char** argv){
  ...
  IndexVector positions(nx*ny*nz);

  Map<float(Index)> f(std::ifstream("kernel_vec.cl"));

  //Begin computation
  positions.setDistribution(distribution::Copy(positions));
  input.setDistribution(distribution::Copy(input));
  
  for (int iter=0; iter<iterations; iter++){
    input = f(positions, input, c0, c1, nx, ny, nz);  
  }
  ...
  //Write output file
  writeGrid(input.begin(), FILE_OUT, nx, ny, nz);
