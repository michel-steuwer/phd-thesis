//+++This file is equal the original file from the Rodinia Benchmark Suite in version 2.3 expect of the lines where comments like this are bordered in "+++". +++  
...
int needleman_wunsch(int argc, char **argv){ //+++change main to needleman_wunsch+++
	...
	auto t1 = std::chrono::high_resolution_clock::now(); //+++insert timestamp+++
	...
	LOG_INFO("Initialize OpenCL: ", std::chrono::duration_cast<std::chrono::milliseconds>(t7 - t6).count(), "ms"); //+++logging the runtime+++
	...
	LOG_INFO("Load kernels: ", std::chrono::duration_cast<std::chrono::milliseconds>((t3 - t2)-(t7 - t6)).count(), "ms"); //+++logging the runtime+++
...
#define TRACEBACK	//+++remove the comment-slashes for enabling the traceback+++
#ifdef TRACEBACK
	...
	auto t5 = std::chrono::high_resolution_clock::now(); //+++insert timestamp+++
